...
class ArticleSection extends StatelessWidget {
  ...
  SearchArticleByTagsBloc searchArticleBloc;
  ...
  Widget build(BuildContext context) {
    ...
    searchArticleBloc=BlocProvider.of<SearchArticleByTagsBloc>(context);
    return Container(
      padding: EdgeInsets.all(10),
      child: BlocBuilder<SearchArticleByTagsBloc,ListArticleTagsState>(
        builder: (ctx,state){
          if(state is ListArticleTagsLoadedState){
            return _buildListArticleTags(state.article, state.isReachedEnd,context);
          }else if(state is ListArticleTagsFailedState){
            print(state.Message);
            return _buildMessage();
          }else if(state is ListArticleTagsUnitializedState){
            return _buildLoading();
          }else if(state is ListArticleTagsEmptyState){
  ...
  Widget _buildListArticleTags(List<ArticleTags>list,bool isReachedEnd,var context){
    return ListView.builder(
        controller: controller,
        itemCount: isReachedEnd?list.length:list.length+1,
        itemBuilder: (contet,index){
          if(index<list.length){
            return _buildRowArticle(list[index].article,context);
          }else{
            return _buildLoading();
          }
        }
